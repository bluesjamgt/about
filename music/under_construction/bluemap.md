...
- 手機與平板不另做窄版瀏覽模式；目前頁面以桌機工作台比例呈現，手機開啟也維持一般網頁視窗感。
- `FORM` 已改為左半控制、右半摘要特寫。右側只顯示 `Main` 層目前 Scale 名稱與 Formula 級數，例如 `A Major Scale` / `1 2 3 4 5 6 7`。
- `Formula` 變成可下拉選單，但顯示文字只放級數；Scale / Variant 仍是資料綁定來源。
- 指板音符顯示級數時，比照 Formula 的級數格式；強調音格式只影響紅框標示，不改指板音符文字。
- `+1` / `-1` 轉調使用短暫滑動動畫，不用淡入殘影，避免視覺疲勞。
- 調色盤移到音名調色盤下方，不影響原本方框工具列。
- 高彩模式有三組差異明顯的預設色與 `U` 使用者自訂 slot；自訂色存在高彩 slot。
- 低彩模式可分別調整外框色、文字色、內圈底色；三組預設底色皆為白色，自訂色存在低彩 slot。
- 高彩與低彩模式都提供透明度拉桿，作用於音點底色，不影響文字與外框清晰度。
- `CLN` 已改成清理類圖示按鈕方向，工具列按鈕位置不因調色盤新增功能而移動。
- `SCOPE` 與位置資訊目前採置底資訊列；左側對齊主畫布起點，快捷鍵說明固定置右，兩者可各自收折並留在原位置。
...
- 吉他弦定義：補上低音延伸與高音延伸命名，例如 `LOWB`、`HIGH E`，避免 7 弦、8 弦或特殊調弦只靠數字猜測。
- 空弦音顯示與自訂：空弦音要可獨立顯示、隱藏與自訂；自訂調弦後，指板音位、弦名標籤與資訊列都要同步更新。
- 音位記號呈現形式：規劃可切換不同 marker 表現，例如純音名、唱名、級數、Formula 級數、低彩空心、全彩實心、強調紅框、選取白框。
- 自訂調弦資料應進入 Persistence，重新整理後保留；未來可整理成 tuning preset。
- 空弦區不應被當成第 0 格重複繪製；它是弦定義與調弦入口，也是音名顯示的一部分。
